...
class M_import extends CI_Model
{
	...
	function tambah_ppareatr($dataarray,$drop)
	{
		...
		for($i=0;$i<count($dataarray);$i++)
		{
			...
			$alamat_ya=str_replace("'","''",$alamat_pel);
			$status_permohonan=str_replace("'","''",$status_permohonan);
			$ket_perluasan=str_replace("'","''",$ket_perluasan);
			$nama_pabrikan_wo_tiang=str_replace("'","''",$nama_pabrikan_wo_tiang);
			$nama_vendorpelak=str_replace("'","''",$nama_vendorpelak);
	...
	function tambah_ppareakol($dataarray,$drop)
	{
		...
		for($i=0;$i<count($dataarray);$i++)
		{
			...
			$nama=str_replace("'","''",$nama_pel);
			$nama_pemohon=str_replace("'","''",$nama_pemohon);
			$alamat=str_replace("'","''",$alamat);
			$status_permohonan=str_replace("'","''",$status_permohonan);
			$ket_perluasan=str_replace("'","''",$ket_perluasan);
			$status_kelayakan=str_replace("'","''",$status_kelayakan);
			$ket_angka=str_replace("'","''",$ket_angka);
			$ket_uraian=str_replace("'","''",$ket_uraian);
			$nama_pabrikan_wo_tiang=str_replace("'","''",$nama_pabrikan_wo_tiang);
			$nama_vendorpelak=str_replace("'","''",$nama_vendorpelak);
			$keterangan=str_replace("'","''",$keterangan);
	...
	function tambah_ppareatm($dataarray,$drop)
	{
		...
		for($i=0;$i<count($dataarray);$i++)
		{
			...
			$nama=str_replace("'","''",$nama_pel);
			$alamat_pel=str_replace("'","''",$alamat_pel);
			$status_permohonan=str_replace("'","''",$status_permohonan);
			$ket_perluasan=str_replace("'","''",$ket_perluasan);
			$status_kelayakan=str_replace("'","''",$status_kelayakan);
			$ket_angka=str_replace("'","''",$ket_angka);
