...
{	
    ...
    $data['total_data_belum_nyala_KOLEKTIF'] = $total_NYALA_KOLEKTIF;
            ...
            //var diagram
    $temp_dg = $this->show_Grafik_HPL();
    $data['HPL_TR']=$temp_dg['HPL_TR'];
    $data['HPL_TM']=$temp_dg['HPL_TM'];
    $data['HPL_KOLEKTIF']=$temp_dg['HPL_KOLEKTIF'];
    $this->template->display('penting',$data);
}
            // fungsi get data grafik
private function show_Grafik_HPL(){
    $this->load->model('m_pelanggantm');
    $this->load->model('m_pelanggantr');
    $this->load->model('m_kolektif');
    ...
    if ($temp_total_data_KOLEKTIF != 0) {
        $HPL_KOLEKTIF[2] = intval(floor($total_HPL_Batu_KOLEKTIF/$temp_total_data_KOLEKTIF));
            ...
            // echo "HPL FIX KOLEKTIF Batu: " .$HPL_KOLEKTIF[2] ."<br>";
    ...
    $temp_total_data_TM = $total_data_TM_GONDANGLEGI->TOTAL;
    $total_data_KOLEKTIF_GONDANGLEGI = $this->m_kolektif->get_total_data_by_rayon_KOLEKTIF('RYN GONDANGLEGI')->row();
    $temp_total_data_KOLEKTIF = $total_data_KOLEKTIF_GONDANGLEGI->TOTAL;      
    ...
    $data['HPL_TR'] = json_encode($HPL_TR);
    $data['HPL_TM'] = json_encode($HPL_TM);
    $data['HPL_KOLEKTIF'] = json_encode($HPL_KOLEKTIF);
    $data['HPL_TR_EX'] = $HPL_TR;
    $data['HPL_TM_EX'] = $HPL_TM;
    $data['HPL_KOLEKTIF_EX'] = $HPL_KOLEKTIF;
...
public function exporttoexcel(){
    ...
    $temp_dg = $this->show_Grafik_HPL();
    $data['HPL_TR_EX']=$temp_dg['HPL_TR_EX'];
    $data['HPL_TM_EX']=$temp_dg['HPL_TM_EX'];
    $data['HPL_KOLEKTIF_EX']=$temp_dg['HPL_KOLEKTIF_EX'];
    $data['jumlah_data_pelanggan_bayar_TR'] = $temp_dg['jumlah_data_pelanggan_bayar_TR'];
    $data['jumlah_data_pelanggan_bayar_TM'] = $temp_dg['jumlah_data_pelanggan_bayar_TM'];
    $data['jumlah_data_pelanggan_bayar_KOLEKTIF'] = $temp_dg['jumlah_data_pelanggan_bayar_KOLEKTIF'];
    $data['total_HPL_TR'] = $temp_dg['total_HPL_TR'];
    $data['total_HPL_TM'] = $temp_dg['total_HPL_TM'];
    $data['total_HPL_KOLEKTIF'] = $temp_dg['total_HPL_KOLEKTIF'];
    $data['nama_rayon'] = $temp_dg['nama_rayon'];
