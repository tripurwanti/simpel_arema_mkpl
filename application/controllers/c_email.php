...
class c_email extends CI_Controller{
	...
	public function send_email_approval($emailuser,$approval){
		$config = [
		...
		$subject = 'PENGAKTIFAN AKUN SIMPEL_AREMA';
		$message = '';
		if ($approval == 'Y') {
			$message = 'Selamat, Anda Sudah Terdaftar di Aplikasi SIMPEL AREMA. Silahkan Buka alamat : 10.5.71.30/simpel_arema/ untuk pergi menuju aplikasi SIMPEL AREMA.';
		}elseif ($approval == 'N') {
			$message = 'Maaf.. Anda Telah ditolak Masuk oleh Admin SIMPEL AREMA karena Anda mungkin adalah Orang yang tidak dikenal atau anda memiliki akun yang lain dengan email yang sama, atau Anda Tidak memenuhi kriteria Anggota SIMPEL AREMA. Terima Kasih...';
		}
		$this->load->library('email', $config);
		$this->email->to(rawurldecode($emailuser));
